...
intraPeriod=$3
interPeriod=$4
# out_dir="/usr/scratch/mayank/brownianNetwork_nocs2019_rslt/$d/sim-type-2/"
...
do
	./build/Garnet_standalone/gem5.opt -d $out_dir/64c/Torus_XY-BN/${routing_algorithm[0]}/${conf_file}/${bench_caps[$b]}/vc-${vc_}/bubble-${bubble}/intra-${intraPeriod}_inter-${interPeriod}/inj-${k} configs/example/garnet_synth_traffic.py --topology=Torus_XY --num-cpus=64 --num-dirs=64 --mesh-rows=8 --network=garnet2.0 --router-latency=1 --sim-cycles=${cycles} --inj-vnet=0 --vcs-per-vnet=${vc_} --conf-file=${conf_file} --sim-type=1 --enable-bn=1 --rand-bb=1 --num-bubble=${bubble} --intra-period=${intraPeriod} --inter-period=${interPeriod} --scheme=bn --injectionrate=${k} --synthetic=${bench[$b]} --routing-algorithm=0 &
done
#sleep 300
done
done
