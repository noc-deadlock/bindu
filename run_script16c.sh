...
do
	./build/Garnet_standalone/gem5.opt -d $out_dir/16c/regular/${routing_algorithm[0]}/${bench_caps[$b]}/vc-${vc_}/bubble-${bubble}/intra-${intraPeriod}_inter-${interPeriod}/inj-${k} configs/example/garnet_synth_traffic.py --topology=Mesh_XY --num-cpus=16 --num-dirs=16 --mesh-rows=4 --network=garnet2.0 --router-latency=1 --sim-cycles=${cycles} --inj-vnet=0 --vcs-per-vnet=${vc_} --sim-type=2 --enable-bn=1 --rand-bb=1 --num-bubble=${bubble} --intra-period=${intraPeriod} --inter-period=${interPeriod} --injectionrate=${k} --synthetic=${bench[$b]} --routing-algorithm=3 &
done
#sleep 300
done
done